...
// SQLite veritabanýna baðlanma
var openDatabaseSuccess = function () {
    console.log("Veritabanýna baþarýyla baðlanýldý.");
    userCreate(function (success) {
        if (success) {
            console.log("db open.");
            // Ýþlemlerinizi burada devam ettirebilirsiniz
        } else {
...
var userCreate = function (callback) {
    db.transaction(function (tx) {
			  tx.executeSql("SELECT name FROM sqlite_master WHERE type='table' AND name='users'", [], function (tx, result) {
				if (result.rows.length < 1) {
						tx.executeSql("CREATE TABLE IF NOT EXISTS users([userID] text,[userName] text,[password] text,[guid] text)", [], function (tx, resp) {
							console.log("Kullanýcýlar tablosu oluþturuldu");
							if (callback) {
								callback(true); // Baþarýyla tamamlandý
							}
						})	
					}
				})	
        ...
        function (error) {
            console.log("Query hatasý: " + error.message);
            if (callback) {
                callback(false); // Hata oluþtu
            }
        });
};
    ...
    try {
        db.transaction(function (tx) {
            tx.executeSql("DROP TABLE IF EXISTS LISTE", [], function () {
                console.log("LISTE tablosu baþarýyla silindi.");
            }, function (tx, error) {
                console.error("LISTE tablosunu silme hatasý: " + error.message);
            });
        },
            function (error) {
                console.error("LISTE tablosunu silme hatasý: " + error.message);
                ...
                db.transaction(function (tx) {
                    tx.executeSql("CREATE TABLE IF NOT EXISTS LISTE(" +
                        ...
                        "[ziyaretAciklama] TEXT)", [], function () {
                            console.log("LISTE tablosu baþarýyla oluþturuldu.");
                            ...
                            console.error("LISTE tablosunu oluþturma hatasý: " + error.message);
        ...
        console.error("Hata oluþtu: " + e.message);
    ...
    try {
        db.transaction(function (tx) {
            tx.executeSql("DROP TABLE IF EXISTS RESPONSES", [], function () {
                console.log("RESPONSES tablosu baþarýyla silindi.");
            }, function (tx, error) {
                console.error("RESPONSES tablosunu silme hatasý: " + error.message);
            });
        },
            function (error) {
                console.error("RESPONSES tablosunu silme hatasý: " + error.message);
